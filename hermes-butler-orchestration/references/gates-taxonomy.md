...
**Behavior:** Loops back to the producer with specific feedback. Bounded by an iteration cap of 3 (the 3-fix limit).

**Recovery:** Producer addresses feedback; checker re-evaluates. The loop escalates early if issue count does not decrease between consecutive iterations (stall detection). After 3 iterations, escalates to the user unconditionally — never loop forever.
